...
const FlagQuiz = () => {
  ...
  const generateQuestions = () => {
    ...
    const quizQuestions = shuffledCountries.slice(0, TOTAL_QUESTIONS).map(country => {
      const allCountryNames = countries.map(c => c.country);
  ...
  if (questions.length === 0) {
    return (
      <QuizLayout title="Flag Quiz" description="Loading questions..." quizType={QUIZ_TYPE}>
  ...
  if (quizCompleted) {
    ...
    return (
      <QuizLayout title="Flag Quiz Complete!" description={`You scored ${currentScore} points! (${percentage}%)`} quizType={QUIZ_TYPE}>
        <motion.div
