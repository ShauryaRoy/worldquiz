...
const CurrencyQuiz = () => {
  ...
  const generateQuestions = () => {
    ...
    const quizQuestions = shuffledCountries.slice(0, TOTAL_QUESTIONS).map(country => {
      const allCurrencies = [...new Set(countries.map(c => c.currency))]; 
      const options = generateUniqueOptions(country.currency, allCurrencies);
      
      return {
        displayElement: (
          <Card className="bg-gradient-to-br from-amber-500/20 to-yellow-600/20 border-white/20">
        ...
        questionText: `What is the currency used in ${country.country}?`,
        options: options.map(opt => ({ label: opt, value: opt })),
        correct: country.currency,
        correctFeedback: `${country.currency} is indeed the currency of ${country.country}!`,
        incorrectFeedback: `The currency of ${country.country} is ${country.currency}.`
      };
    });
  ...
  if (questions.length === 0) {
    return (
      <QuizLayout title="Currency Quiz" description="Loading questions..." quizType={QUIZ_TYPE}>
  ...
  if (quizCompleted) {
    ...
    return (
      <QuizLayout title="Currency Quiz Complete!" description={`You scored ${currentScore} points! (${percentage}%)`} quizType={QUIZ_TYPE}>
        <motion.div
  ...
  return (
    <QuizLayout 
      title="Currency Quiz" 
      description="Match the country with its currency."
      quizType={QUIZ_TYPE}
    >
      <QuizQuestion
