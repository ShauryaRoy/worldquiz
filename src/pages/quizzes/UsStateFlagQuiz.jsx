...
const QUIZ_TYPE = "us-state-flags";
const TOTAL_QUESTIONS = 10; // Or usStates.length for all states

const UsStateFlagQuiz = () => {
  ...
  const generateQuestions = () => {
    ...
    const numQuestions = Math.min(TOTAL_QUESTIONS, usStates.length);
    const quizQuestions = shuffledStates.slice(0, numQuestions).map(state => {
      ...
      return {
        image: state.flag,
        imageAlt: `Flag of ${state.name}`,
        questionText: "Which US state does this flag belong to?",
        options: options.map(opt => ({ label: opt, value: opt })),
        correct: state.name,
        correctFeedback: `That's the flag of ${state.name}!`,
        incorrectFeedback: `That was the flag of ${state.name}.`
      };
    });
  ...
  if (questions.length === 0) {
    return (
      <QuizLayout title="US State Flag Quiz" description="Loading questions..." quizType={QUIZ_TYPE}>
  ...
  if (quizCompleted) {
    ...
    return (
      <QuizLayout title="US State Flag Quiz Complete!" description={`You scored ${currentScore} points! (${percentage}%)`} quizType={QUIZ_TYPE}>
        <motion.div
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="max-w-md mx-auto text-center"
        >
          <Card className="bg-gradient-to-r from-blue-500/20 to-red-500/20 border-white/20">
  ...
  return (
    <QuizLayout 
      title="US State Flag Quiz" 
      description="Identify the US state by its flag."
      quizType={QUIZ_TYPE}
    >
      <QuizQuestion
...
export default UsStateFlagQuiz;