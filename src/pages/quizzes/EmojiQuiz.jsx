...
const EmojiQuiz = () => {
  ...
  const generateQuestions = () => {
    const countriesWithEmojis = countries.filter(c => c.emoji && c.emoji.trim() !== "");
    const shuffledCountries = [...countriesWithEmojis].sort(() => Math.random() - 0.5);
    
    const quizQuestions = shuffledCountries.slice(0, TOTAL_QUESTIONS).map(country => {
      const allCountryNames = countriesWithEmojis.map(c => c.country);
      const options = generateUniqueOptions(country.country, allCountryNames);
      
      return {
        displayElement: (
          <Card className="bg-gradient-to-br from-yellow-500/20 to-orange-600/20 border-white/20">
        ...
        questionText: "Which country do these emojis represent?",
        options: options.map(opt => ({ label: opt, value: opt })),
        correct: country.country,
        correctFeedback: `Those emojis represent ${country.country}!`,
        incorrectFeedback: `Those emojis represent ${country.country}.`
      };
    });
  ...
  if (questions.length === 0) {
    return (
      <QuizLayout title="Emoji Country Quiz" description="Loading emoji questions..." quizType={QUIZ_TYPE}>
        <div className="flex justify-center py-10">
          <div className="animate-spin rounded-full h-32 w-32 border-b-4 border-orange-400"></div>
        </div>
      </QuizLayout>
    );
  }
  ...
  if (quizCompleted) {
    ...
    return (
      <QuizLayout title="Emoji Quiz Complete!" description={`You scored ${currentScore} points! (${percentage}%)`} quizType={QUIZ_TYPE}>
        <motion.div
          ...
          <Card className="bg-gradient-to-r from-green-500/20 to-blue-500/20 border-white/20">
            <CardContent className="p-8">
              ...
              <h2 className="text-3xl font-bold text-white mb-2">
                {percentage >= 80 ? 'Emoji Master!' : percentage >= 60 ? 'Great Job!' : 'Good Effort!'}
              </h2>
  ...
  return (
    <QuizLayout 
      title="Emoji Country Quiz" 
      description="Decode the emojis to find the country."
      quizType={QUIZ_TYPE}
    >
      <QuizQuestion
