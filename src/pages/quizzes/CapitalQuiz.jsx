...
const CapitalQuiz = () => {
  ...
  const generateQuestions = () => {
    ...
    const quizQuestions = shuffledCountries.slice(0, TOTAL_QUESTIONS).map(country => {
      ...
      return {
        ...
        questionText: `What is the capital city of ${country.country}?`,
        options: options.map(opt => ({ label: opt, value: opt })),
        correct: country.capital,
        correctFeedback: `${country.capital} is indeed the capital of ${country.country}!`,
        incorrectFeedback: `The capital of ${country.country} is ${country.capital}.`
      };
    });
  ...
  if (questions.length === 0) {
    return (
      <QuizLayout title="Capital Cities Quiz" description="Loading questions..." quizType={QUIZ_TYPE}>
  ...
  if (quizCompleted) {
    ...
    return (
      <QuizLayout title="Capital Quiz Complete!" description={`You scored ${currentScore} points! (${percentage}%)`} quizType={QUIZ_TYPE}>
        <motion.div
